export const FLAG_CATEGORIES = ['学习', '健身', '阅读', '项目', '自定义']
...
export const FREQUENCY_TIMES_OPTIONS = [1, 2, 3, 4, 5, 6, 7]

export function parseCheckinFrequency(freq) {
	if (freq === 'daily') return { period: 'day', times: 1 }
	if (freq === 'weekly3') return { period: 'week', times: 3 }
...
export function formatCheckinFrequency(freq) {
	if (freq === 'daily') return '每天'
	if (freq === 'weekly3') return '每周 3 次'
	const parsed = parseCheckinFrequency(freq)
	const periodMap = { day: '日', week: '周', month: '月' }
	return `每${periodMap[parsed.period] || '日'} ${parsed.times} 次`
}

export function buildCustomFrequency(period, times) {
